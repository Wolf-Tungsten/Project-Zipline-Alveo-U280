...
set kernel_name    "cceip_kernel"
set kernel_vendor  "gaoruihao.ncic.ict.cas"
...
proc edit_core {core} {
  ...
  set reg      [::ipx::add_register -quiet "input_size" $addr_block]
  set_property address_offset 0x010 $reg
  set_property size           [expr {8*8}]   $reg

  set reg      [::ipx::add_register -quiet "input_addr" $addr_block]
  set_property address_offset 0x01c $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m00_axi $regparam 

  set reg      [::ipx::add_register -quiet "output_size_addr" $addr_block]
  set_property address_offset 0x028 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m00_axi $regparam 

  set reg      [::ipx::add_register -quiet "output_addr" $addr_block]
